namespace Game.Maps
{
	...
	[Serializable]
	public abstract class CellsBase<T> : ICells<T>
	{
...
#region Constructors
		...
		protected CellsBase( int width, int height, T cell )			: this( new Vector2Int(width, height), cell ) {}
		protected CellsBase( int width, int height )					: this( new Vector2Int(width, height) ) {}
		protected CellsBase( Vector2Int size )						: this( new RectInt( Vector2Int.zero, size ) ) {}


		protected CellsBase( Vector2Int size, T cell )		: this( size )
		{
			SetAll( cell );
		}

		protected CellsBase( CellsBase<T> other )				: this( other._rect )
		{
			_cells		= (T[,])other._cells.Clone();
		}

		protected CellsBase( RectInt rect )
		{
			_rect		= rect;
		...
		public void Set( CellsBase<T> other )
		{
			if (Size != other.Size)
		...
		public void Set( ICells<T> other, Vector2Int dstMin )
		{
			Set( other, new RectInt( Vector2Int.zero, other.Size ), dstMin );
		}


		public void Set( ICells<T> other, RectInt srcRect, Vector2Int dstMin = default )
		{
